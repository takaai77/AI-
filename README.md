...
│       ├── cancel-order.ts
│       ├── get-customer-info.ts
│       └── skip-next-delivery.ts
...
| アクション | 説明 | 必須パラメータ |
|-----------|------|---------------|
| `check_order_status` | 注文ステータス確認 | customerId |
| `cancel_order` | 注文キャンセル | customerId, orderNo |
| `get_customer_info` | 顧客情報取得 | customerId |
| `skip_next_delivery` | 次回配送スキップ | customerId |
